...
export const getBidsData = async (
  ...
  try {
    const bids = await fetch(
      `https://5005-alenkassemiro-housy-6qhmsg4ys2y.ws-us102.gitpod.io/inspect/auctions/${id}/bids`
    );
...
export const getAuctions = async (): Promise<Auction[] | null> => {
  try {
    const response = await fetch("https://5005-alenkassemiro-housy-6qhmsg4ys2y.ws-us102.gitpod.io/inspect/auctions");
